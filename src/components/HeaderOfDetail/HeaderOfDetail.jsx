...
import { actFetchDetail, actMoreDetail, getCurrentDate, getCurrentLocation, getGuest } from '../../containers/PageDetail/module/action';
export default function Header(props) {
    ...
    const [toStay, setToStay] = useState(false)
    ...
    const solveTransition = () => {
       ...
       dispatch(getGuest(totalPeople))
       dispatch(getCurrentLocation(isPlace))
       dispatch(getCurrentDate(moment(StartDate).format("MMM D"),moment(EndDate).format("MMM D")))
       setIsVisible(false);
    }
    ...
    return location && (
        <div>
            <div className={`header ${isVisible ? 'active' : ''} ${isHighlight ? 'isHeader' : ''} headerSec`} style={{ position: 'fixed', zIndex: "1000000000!important" }}>
                <nav className="navbar navbar-expand-lg navbar-light bg-white py-3 navbar__detail">
                        ...
                        <div className="collapse navbar-collapse" id="navbarSupportedContent">
                            {
                                ...
                                <div className="navbar-nav input__value">
                                    <div className="detailofPage" onClick={() => showOption()}>
                                        <span>{locationCurrent}</span>
                                        <span>{moment(startOfDate).format("MMM D")} - {moment(endOfDate).format("MMM D")}</span>
                                        <span className="guest_detail">{guest} guests</span>
                                        <span className="guest_detail icon__detail"> <i class="fas fa-search"></i></span>
                                    </div>
                                </div>
                            </ul>
                            }
                            <form className="d-flex ml-auto navbar__right">
                                <a className="nav-link-host" style={{ color: 'black' }}>Become a host</a>
                                <a>
                                    <svg viewBox="0 0 16 16" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="presentation" focusable="false" style={{ display: "block", height: '16px', width: "16px", fill: "black", fontWeight: 'bold', marginLeft: '-13px' }}>
                                ...
                                {
                                    userSignIn !== null ? (
                                        <h1 style={{ height: '40px', width: '40px', borderRadius: '50%', backgroundImage: `url(${JSON.parse(userSignIn)?.avatar})`, backgroundPosition: 'center', backgroundRepeat: 'no-repeat', backgroundSize: 'cover', border: '0.4px solid #ccc' }} onClick={() => setModal(true)}></h1>
                            ...
                            <div className="row">
                                ...
                                <div className="col-6">
                                    <div className="selection" style={{paddingBottom:'15px'}}>
                                        ...
                                        <div className="signIn" onClick={() => setShowModal()}><a to="/">Sign in</a></div>
                                        <div className="signUp" onClick={() =>  setShowSignUp()}><a to="/">Sign up</a></div>
